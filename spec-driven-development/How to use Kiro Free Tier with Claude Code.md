...
- **Download Kiro IDE:** Go to kiro.dev (or search for Kiro IDE) and download the application.
- **Sign Up & Login:** Open Kiro IDE and sign in.
...
- **Clone the Gateway Repository:** Open your terminal/command prompt and run:
...
- **Install the Tools:** Run these commands to install the official Claude Code CLI and the Router:
...
- **Configure the Router:** Create (or edit) the configuration file at ~/.claude-code-router/config.json. Paste this configuration:
...
- **Start the Router Service:** In a new terminal window, start the router background service:

```bash
ccr start
```

- **Run Claude Code:** Now, instead of running claude, run:

```bash
ccr code
```

- **Test it out** by typing a prompt, for example:
